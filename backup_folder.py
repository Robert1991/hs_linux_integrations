...
def zipdir(directory_path, ziph):
    for root, _, files in walk(directory_path):
        for file in files:
            try:
                ziph.write(path.join(root, file), path.relpath(
                    path.join(root, file), path.join(directory_path, '..')))
            except:
                caught = sys.exc_info()[0]
                print("Unexpected error:" + str(caught) +
                      " at file: " + str(file))
...
backup_file_path = path.join(
    target_folder, create_backup_zip_file_name(source_folder))
