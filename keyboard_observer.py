...
with open(sys.argv[1]) as configuration_file:
  ...
  state_topic = create_state_topic(config_data, "keyboard_observer")
  
  client = connect_mqtt_client(config_data["connection_data"], config_data["login"]) 
  auto_configure_motion_sensor(client, config_data, "keyboard_observer")

  keyboard_event_file = find_event_file(config_data["keyboard_observer"]["observed_dev_event_id"])
  keyboard_event_type = config_data["keyboard_observer"]["observed_dev_event_type"]
